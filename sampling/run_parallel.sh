...
# The continuation family has its own sweep: sampling_new/run_parallel.sh.
#
# Usage:
#   bash sampling/run_parallel.sh <model_name> <port> [n_seeds=25] [reasoning=false] [temperature=1.0]
#
# model_name must be listed in MODEL_NAME_TO_TYPE (sampling/utils.py); the
...
run_config() {
    ...
    for SEED in $(seq 0 $(($NSEEDS - 1))); do
        ...
        launch_field $COMMON_ARGS $SEED_ARGS --methods direct --gibbs_k_vars 16
