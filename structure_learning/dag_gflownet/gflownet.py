...
class DAGGFlowNet:
    ...
    def __init__(self, model=None, delta=1.0, update_target_every=1000):
        if model is None:
            model = gflownet
    ...
    def loss(self, params, target_params, samples):
        ...
        log_pi_tp1 = vmodel(target_params, samples["next_adjacency"], samples["next_mask"])
    ...
    def act(self, params, key, adjacency, mask, epsilon):
        ...
        log_pi = vmap(self.model.apply, in_axes=(None, 0, 0))(params, adjacencies, masks)

        # Get uniform policy
        log_uniform = uniform_log_policy(masks)

        # Mixture of GFlowNet policy and uniform policy
        is_exploration = random.bernoulli(subkey1, p=1.0 - epsilon, shape=(batch_size, 1))
    ...
    def step(self, params, state, samples):
        grads, logs = grad(self.loss, has_aux=True)(params.online, params.target, samples)
        ...
        params = DAGGFlowNetParameters(
            ...
            target=optax.periodic_update(
                online_params, params.target, state.steps, self.update_target_every
            ),
        )
    ...
    def init(self, key, optimizer, adjacency, mask):
        ...
        params = DAGGFlowNetParameters(online=online_params, target=online_params)
