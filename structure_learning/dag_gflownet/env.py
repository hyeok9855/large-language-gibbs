...
class GFlowNetDAGEnv(gym.vector.VectorEnv):
    def __init__(
        self, num_envs, scorer, max_parents=None, num_workers=4, context=None, cache_max_size=10_000
    ):
        ...
        observation_space = Dict(
            {
                "adjacency": Box(low=0.0, high=1.0, shape=shape, dtype=np.int_),
                "mask": Box(low=0.0, high=1.0, shape=shape, dtype=np.int_),
    ...
    def step(self, actions):
        ...
        if not np.all(self._state["mask"][~dones, sources, targets]):
            raise ValueError(
                "Some actions are invalid: either the edge to be "
    ...
    def local_scores_async(self, sources, targets):
        ...
        for i, (source, target) in enumerate(zip(sources, targets)):
            if source == self.num_variables:
                ...
                indices = tuple(
                    index for index, is_parent in enumerate(adjacency[:, target]) if is_parent
                )
                ...
                if not self._is_in_cache((target, indices_after), local_cache):
                    if not self._is_in_cache((target, indices), local_cache):
                        ...
                        local_cache.update({(target, indices), (target, indices_after)})
                    else:
    ...
    def local_scores_wait(self, keys, local_cache, data):
        ...
        if self.num_workers > 0:
            ...
            for target, indices, indices_after in data:
                local_score_before = self.scorer.local_score(target, indices)
                self.local_scores[local_score_before.key] = (
                    local_score_before.score + local_score_before.prior
                )

                if indices_after is not None:
                    local_score_after = self.scorer.local_score(target, indices_after)
        ...
        for target, key_tm1, key_t in keys:
            if target is None:
                ...
                delta_scores.append(
                    self.local_scores[(target, key_t)] - self.local_scores[(target, key_tm1)]
                )
    ...
    def close_extras(self, **kwargs):
        ...
        try:
            ...
            for q in (self.in_queue, self.out_queue, self.error_queue):
                try:
                    q.close()
                    q.join_thread()
                except Exception:
