...
def to_graphs_tuple(adjacencies, pad=True):
    ...
    n_node = np.full((num_graphs,), num_variables, dtype=np.int64)
...
def batch_sequences_to_graphs_tuple(num_variables, actions, lengths):
    ...
    senders = np.where(
        offsets <= lengths[:, None], senders + offsets * num_variables, max_length * num_variables
    )
    receivers = np.where(
        offsets <= lengths[:, None], receivers + offsets * num_variables, max_length * num_variables
    )
