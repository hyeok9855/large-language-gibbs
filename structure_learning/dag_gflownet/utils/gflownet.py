...
def detailed_balance_loss(log_pi_t, log_pi_tp1, actions, delta_scores, num_edges, delta=1.0):
    ...
    error = (
        ...
        + log_pi_t[:, -1]
        - lax.stop_gradient(log_pi_tp1[:, -1])
    )
...
def posterior_estimate(gflownet, params, env, key, num_samples=1000, verbose=True):
    ...
    with trange(
        num_samples, desc="[posterior_estimate]", disable=(not verbose), dynamic_ncols=True
    ) as pbar:
